...
```{r PLOT TRUE COLOR, echo=FALSE, eval=FALSE}
#iterate 
...
```{r PANSHARPEN, echo=TRUE, eval=TRUE}
...
```{r, echo=TRUE, eval=TRUE}
test_image <- list_of_rasters_renamed[[6]]
...
test_ndvi <- (r_stretched$B8- r_stretched$red) / 
             (r_stretched$B8 + r_stretched$red)
summary(test_ndvi)
...
```{r EQUATIONS, echo=TRUE, eval=TRUE}
